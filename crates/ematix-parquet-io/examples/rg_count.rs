...
fn main() {
    ...
    let mut entries: Vec<_> = std::fs::read_dir(base).unwrap().map(|e| e.unwrap().path()).collect();
    entries.sort();
    for p in entries {
        if p.extension().and_then(|s| s.to_str()) == Some("parquet") {
            ...
            println!("{:20}  rows={:>10}  rgs={}", p.file_name().unwrap().to_string_lossy(), total_rows, md.row_groups.len());
        }
    }
}
